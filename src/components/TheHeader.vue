...
import { Paintbrush, Check, Sun, Moon } from 'lucide-vue-next'
...
<template>
  <header class="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
    <div class="container flex h-14 max-w-screen-2xl items-center justify-between px-4">
      ...
      <!-- ЛЕВАЯ ЧАСТЬ: Логотип + Кнопки управления -->
              ...
              <div class="grid grid-cols-3 gap-1.5">
                ...
                >
                  <span :class="[color.class, 'h-5 w-5 rounded-full shrink-0 flex items-center justify-center']">
                    <Check v-if="activeTheme === color.value" class="h-3 w-3 text-white" stroke-width="3" />
                  </span>
      ...
      <!-- ПРАВАЯ ЧАСТЬ: Навигация (скрыта на мобильных, видна на ПК) -->
      <nav class="hidden md:flex items-center gap-1">
        ...
        <Button variant="ghost" as-child>
          <a href="#skills" class="text-muted-foreground hover:text-primary transition-colors">
            Навыки
          </a>
        </Button>
        <Button variant="ghost" as-child>
          <a href="#experience" class="text-muted-foreground hover:text-primary transition-colors">
            Опыт
          </a>
        </Button>
        <Button variant="ghost" as-child>
        ...
        <Button variant="ghost" as-child>
          <a href="#contacts" class="text-muted-foreground hover:text-primary transition-colors">
            Контакты
          </a>
        </Button>
      </nav>
